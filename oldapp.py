...
def home():
    if request.method == "POST":
        if "login" in request.form:
            ...
            # Yeni kullanıcı için limit kaydı oluştur
            new_limit = Limit(
                user_id=new_user.id,
                is_premium=False,
                daily_limit_photo=1,
...
@login_required
def user_guide():
    return redirect(url_for('static', filename='uploads/USER_GUIDELINE.pdf'))
...
def view_history():
    ...
    # Şu anki kullanıcının yüklediği medyaları alın
...
@login_required
def my_history():
    # Kullanıcının yüklediği tüm medya dosyalarını al
    media_files = Media.query.filter_by(user_id=current_user.id).all()
...
def upload_video():
    ...
    # Eğer kullanıcıya ait limit yoksa oluştur5
    if not user_limit:
        user_limit = Limit(
            user_id=current_user.id,
    ...
    if file:
        ...
        # 💥 Bu kısmı kaldırıyoruz → aynı dosya tekrar yüklensin:
        # existing_media = Media.query.filter_by(file_path=f"static/uploads/{filename}").first()
        # if existing_media:
        #     flash("File already exists.", "warning")
        #     return redirect(url_for('view_media', file_path=existing_media.file_path, media_type=existing_media.media_type))
        ...
        try:
            ...
            if not current_user.is_premium:
                ...
                db.session.add(user_limit)  # 🟢 Çok önemli — sayaç commitlenmezse güncellenmez!
...
def reset_password(token):
    try:
        # Token’dan e-postayı çıkar (1 saat geçerli)
    ...
    if request.method == 'POST':
        ...
        user.password = hashed_password.decode('utf-8')  # Veya user.set_password(new_pw)
        db.session.commit()
    ...
    if request.method == 'POST':
        ...
        if file and file.filename != '':
            ...
            if ext in ['.jpg', '.jpeg', '.png']:
                ...
                print("Kaydedilecek:", os.path.join(image_folder, filename))
                print("Dosya geldi:", file.filename)
...
def process_media(file_path):
    ...
    if file_path.lower().endswith(('mp4', 'avi', 'mov')):
        ...
        skip_rate = 10  # Her 10. kareyi işle
        frame_index = 0
...
def process_frame(frame, model):
    ...
    cv2.putText(frame, f'People Count: {head_count}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
...
def generate_frames(file_path): # kişi sayımı
    ...
    # Fotoğraf mı yoksa video mu kontrol et
...
def generate_camera_frames():
    ...
    # Öncelikle index=1 (USB) dene, yoksa index=0 (laptop)
    cap = None
    for index in [1, 0]:
        test_cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)
        if test_cap.isOpened():
            cap = test_cap
            print(f"Kullanılan kamera indexi: {index}")
            break
    if cap is None:
        print("Kamera bulunamadı.")
        return

  #pc kamerası hızlandırmak için live-camera görüntüsünü
    frame_skip = 1
    frame_count = 0
...
def start_ip_camera():
    ...
    return redirect(url_for("ip_camera_feed"))
...
def start_camera():
    ...
    if not current_user.is_premium:
        ...
        # 24 saatlik süre doldu mu kontrol et
        if datetime.utcnow() - user_limit.last_reset_date > timedelta(days=1):
        ...
        db.session.commit()
        log_action(current_user.id, "Camera started.") #LOG TABLOSU AKTİFLEŞTİRME
        
#########################################################################################
    user_agent = request.headers.get('User-Agent', '').lower()
    if "mobile" in user_agent:
        print("Mobil cihaz algılandı → phone_camera.html açılıyor")
        return render_template("phone_camera.html")
    else:
        print("PC → camera_detection.html açılıyor")
